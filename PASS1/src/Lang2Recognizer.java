public class Lang2Recognizer extends LanguageRecognizer{
	...
	{
		try {
			if(!ID()) return false;						//parse ID
			if(getCurrentToken() != '=') return false;	//make sure the token is now '='
			getNextToken();								//get next token
			if(!EXPR()) return false;					//parse EXPR
			
			if(isEndOfSentence()) return true;			//if this is the end of the string... We did it!
			else return false;							//Otherwise, yell at them
	...
	private boolean EXPR() throws EndOfSentenceException
	{
		if(DIGIT())	//parse DIGIT
		{
			...
			{
				getNextToken();				//Get next token
				if(EXPR()) return true;		//if EXPR return true, else fall through
			} else return true;
		}
